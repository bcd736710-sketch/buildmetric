...
export function ProductDetailPage({ product }: { product: Product }) {
  ...
  return <main className="bg-white text-navy">
    ...
    <section className="border-t border-navy/10 px-5 py-16 sm:px-8 lg:px-12 lg:py-28"><div className="mx-auto max-w-[1080px]">
      ...
      <article className="mt-20 border-t border-navy/15 pt-10 sm:mt-28 sm:pt-12"><h2 className="text-4xl font-semibold tracking-[-0.04em] text-navy sm:text-5xl">Wholesale / OEM / Project Supply</h2>{wholesaleSupplyDescription.length ? <div className="mt-7 max-w-[800px] space-y-5 text-base leading-8 text-slate sm:text-lg">{wholesaleSupplyDescription.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div> : null}{seo?.buyerNotes ? <section className="mt-12 max-w-[900px] border-t border-navy/10 pt-10 sm:mt-16 sm:pt-12"><h2 className="text-3xl font-semibold tracking-[-0.035em] text-navy sm:text-4xl">B2B Buyer Notes</h2><p className="mt-6 max-w-[800px] text-base leading-8 text-slate sm:text-lg">{seo.buyerNotes.summary}</p><div className="mt-9 grid gap-8 lg:grid-cols-2 lg:gap-12"><section><h3 className="text-xl font-semibold tracking-tight text-navy">Published Options</h3><p className="mt-4 leading-7 text-slate">{seo.buyerNotes.publishedOptions}</p></section><section><h3 className="text-xl font-semibold tracking-tight text-navy">Confirm Before Sampling</h3><ul className="mt-4 space-y-3 text-slate">{seo.buyerNotes.confirmBeforeSampling.map((item) => <li className="flex gap-3 leading-7" key={item}><span aria-hidden="true" className="mt-[0.55em] h-1.5 w-1.5 shrink-0 rounded-full bg-forest" />{item}</li>)}</ul><p className="mt-5 leading-7 text-slate">{seo.buyerNotes.confirmationNote}</p></section></div><section className="mt-10 border-t border-navy/10 pt-8"><h3 className="text-xl font-semibold tracking-tight text-navy">Related Buyer Resources</h3><ul className="mt-5 space-y-4">{seo.buyerNotes.resources.map((resource) => <li key={resource.href}><Link className="font-semibold text-forest underline decoration-forest/35 underline-offset-4 transition hover:text-navy" href={resource.href}>{resource.label}</Link><p className="mt-1 leading-7 text-slate">{resource.description}</p></li>)}</ul></section></section> : null}<div className="mt-9 flex flex-wrap gap-4"><Link className={trovaneButton.primary} href={rfq}>Get a Wholesale Quote</Link><Link className={trovaneButton.secondary} href={sampleRfq}>Request a Sample</Link></div></article>
    </div></section>
  </main>;
}
