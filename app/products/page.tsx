...
const categories = [
  {
    ...
    summary: "Carriers, restraints and comfort products for pet travel.",
  },
  {
    title: "Walking & Hiking",
    href: "/products/travel-car",
    image: "/trovane-category-hiking-dog.jpg",
    summary: "Harnesses, leashes and active outdoor walking essentials.",
  },
  {
    title: "Outdoor Feeding",
    href: "/products/travel-car",
    image: "/trovane-product-bowl-cat.jpg",
    summary: "Portable hydration and feeding products for days outside.",
  },
  {
    title: "Outdoor Apparel",
    href: "/products/travel-car",
    image: "/trovane-category-apparel-dog.jpg",
    summary: "Functional layers for weather, comfort and movement.",
  },
  {
    title: "Camping & Accessories",
    href: "/products/travel-car",
    image: "/trovane-category-camping-pets.jpg",
    summary: "Travel mats, portable beds and light-camping accessories.",
  },
];
    ...
    <header className="sticky top-0 z-50 border-b border-navy/10 bg-warm/92 backdrop-blur">
      <div className="mx-auto flex h-[68px] max-w-[1360px] items-center justify-between gap-3 px-4 sm:px-6 lg:gap-5 lg:px-8">
        ...
        <nav className="hidden items-center gap-6 text-[13px] font-semibold text-navy/78 lg:flex">
          ...
          <Link className="hover:text-forest" href="/#contact">
            Contact
          </Link>
        </nav>
        <Link
          className="hidden min-h-12 items-center justify-center rounded-full bg-forest px-6 text-sm font-bold uppercase tracking-[0.08em] text-white transition hover:bg-navy lg:inline-flex"
          href="/rfq"
        >
          Request a Quote
        </Link>
...
export default function ProductsPage() {
  return (
    <main className="bg-warm text-navy">
      ...
      <section className="px-4 pb-10 pt-10 sm:px-6 sm:pb-12 sm:pt-12 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-8 text-sm text-slate">Products</div>
          <h1 className="max-w-3xl text-4xl font-semibold tracking-tight text-navy sm:text-6xl">
            Explore TROVANE Products
          </h1>
          <p className="mt-5 max-w-2xl text-base leading-8 text-slate">
            Outdoor and travel products for dogs and cats, organized by use case.
          </p>
        </div>
      </section>
      ...
      <section className="px-4 pb-20 sm:px-6 sm:pb-28 lg:px-8">
        <div className="mx-auto flex max-w-7xl flex-wrap justify-center gap-5">
          {categories.map((category) => (
            <Link
              className="group w-full border border-navy/10 bg-warm transition duration-300 hover:border-forest/35 sm:w-[calc((100%-1.25rem)/2)] lg:w-[calc((100%-2.5rem)/3)]"
              href={category.href}
              key={category.title}
            >
              <div className="relative aspect-[4/3] overflow-hidden bg-mist">
                <Image
                  alt={`${category.title} pet outdoor product category`}
              ...
              <div className="flex min-h-[190px] flex-col px-5 py-5 sm:px-6 sm:py-6">
                ...
                <span className="mt-auto inline-flex pt-5 text-sm font-bold uppercase tracking-[0.08em] text-forest">
                  View Category -&gt;
                </span>
              </div>
            </Link>
