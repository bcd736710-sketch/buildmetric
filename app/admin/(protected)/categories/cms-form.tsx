...
export function CategoryCmsForm({ item }: { item?: ProductCategory }) {
  ...
  return <><main className="admin-shell"><form action={action} className="admin-card admin-form wide">
    ...
    {item ? <CategoryImageManager categoryId={item.id} categoryName={item.name} imageAlt={item.imageAlt} imageUrl={item.imageUrl} /> : <p className="admin-image-note">Save this category first, then upload and crop its 1:1 cover image.</p>}
