...
function quote(value: { quotedPrice: string | null; currency: string | null }) {
  return value.quotedPrice ? [value.quotedPrice, value.currency].filter(Boolean).join(" ") : "—";
}

export default async function RFQListPage() {
  await requireAdmin();
  const submissions = await getRFQSubmissions();
  return <main className="admin-shell"><section className="admin-card wide"><p className="admin-kicker">TROVANE</p><h1>RFQs</h1><p><Link href="/admin">Admin</Link> · <Link href="/admin/products">Products</Link> · <Link href="/admin/categories">Categories</Link></p>{submissions.length ? <table><thead><tr><th>Status</th><th>Customer</th><th>Company</th><th>Email</th><th>Product</th><th>Quantity</th><th>Country</th><th>Quote</th><th>Next Follow-up</th><th>Received</th><th /></tr></thead><tbody>{submissions.map((submission) => <tr key={submission.id}><td><span className="admin-status">{submission.status}</span></td><td>{submission.record.name}</td><td>{submission.record.company || "—"}</td><td><a href={`mailto:${submission.record.email}`}>{submission.record.email}</a></td><td>{submission.record.product}</td><td>{submission.record.quantity}</td><td>{submission.record.country}</td><td>{quote(submission.quote)}</td><td>{followUpDate(submission.nextFollowUpAt)}</td><td>{date(submission.createdAt)}</td><td><Link href={`/admin/rfq/${encodeURIComponent(submission.id)}`}>View</Link></td></tr>)}</tbody></table> : <p>No RFQ submissions yet.</p>}</section></main>;
}
