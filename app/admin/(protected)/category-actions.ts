...
export async function saveCategoryCms(_: CategorySaveState, form: FormData): Promise<CategorySaveState> {
  ...
  try {
    ...
    if (id) await database().query(`UPDATE product_categories SET name=$1, slug=$2, description=$3,
      ...
      updated_at=now() WHERE id=$10`, [name, slug, text(form, "description") || null, imageUrl,
      sortOrder(form), status, text(form, "seoTitle") || null, text(form, "seoDescription") || null, status === "published", id]);
    else {
      const rows = await database().query(`INSERT INTO product_categories(name, slug, description, image_url,
        sort_order, status, seo_title, seo_description, is_active) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id`,
      [name, slug, text(form, "description") || null, imageUrl, sortOrder(form), status,
        text(form, "seoTitle") || null, text(form, "seoDescription") || null, status === "published"]) as Array<{ id: string }>;
      savedId = rows[0]?.id || "";
    }
  } catch (error) {
  ...
  redirect(savedId ? `/admin/categories/${savedId}/edit` : "/admin/categories");
}
