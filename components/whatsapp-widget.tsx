  ...
  return <aside aria-label="WhatsApp chat" className="fixed bottom-[max(16px,env(safe-area-inset-bottom))] right-4 z-[9999] sm:bottom-6 sm:right-6">
    {isOpen ? <section aria-labelledby="whatsapp-widget-title" className="flex h-[min(450px,calc(100dvh-8rem))] w-[calc(100vw-2rem)] max-w-[360px] flex-col overflow-hidden rounded-[18px] border border-black/10 bg-white shadow-[0_16px_48px_rgba(20,37,31,0.24)] sm:h-[480px]" id="whatsapp-widget">
      ...
      <form className="relative shrink-0 bg-[#f0f2f5] px-2.5 py-2.5" onSubmit={sendMessage}>
        ...
        <div className="flex items-center gap-2"><button aria-controls="whatsapp-emoji-picker" aria-expanded={isPickerOpen} aria-label="Open emoji picker" className="flex h-12 w-12 shrink-0 touch-manipulation items-center justify-center rounded-full text-[#54656f] transition hover:bg-black/5 focus:outline-none focus:ring-2 focus:ring-[#25D366]" onClick={() => setIsPickerOpen((open) => !open)} type="button"><SmileIcon /></button><label className="sr-only" htmlFor="whatsapp-message">Write your message</label><input className="h-12 min-w-0 flex-1 rounded-full bg-white px-4 text-sm text-[#182a24] outline-none placeholder:text-[#667781] focus:ring-2 focus:ring-[#25D366]" id="whatsapp-message" onChange={(event) => setMessage(event.target.value)} placeholder="Write your message..." ref={inputRef} type="text" value={message} /><button aria-label="Send WhatsApp message" className="flex h-12 w-12 shrink-0 touch-manipulation items-center justify-center rounded-full bg-[#25D366] text-white transition hover:bg-[#128C7E] focus:outline-none focus:ring-2 focus:ring-[#128C7E] focus:ring-offset-2" type="submit"><SendIcon /></button></div>
      </form>
