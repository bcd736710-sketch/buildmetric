...
const defaultMessage = "Hi TROVANE, I would like to know more about your pet outdoor products.";
...
export function WhatsAppWidget() {
  ...
  function sendMessage(event: FormEvent<HTMLFormElement>) {
    ...
    const number = (process.env.NEXT_PUBLIC_TROVANE_WHATSAPP_NUMBER || "").replace(/\D/g, "");
    if (!number) {
  ...
  return <aside className="fixed bottom-5 right-4 z-[60] flex flex-col items-end gap-3 sm:bottom-6 sm:right-6" aria-label="WhatsApp chat">
    {isOpen ? <section aria-labelledby="whatsapp-widget-title" className="w-[calc(100vw-2rem)] max-w-[360px] overflow-hidden rounded-2xl border border-navy/10 bg-white shadow-[0_18px_50px_rgba(20,37,31,0.2)]" id="whatsapp-widget">
      ...
      <div className="bg-[#f3f6f4] px-5 py-6"><div className="max-w-[252px] rounded-2xl rounded-tl-sm bg-white px-4 py-3 text-sm leading-6 text-navy shadow-sm">Hi <span aria-hidden="true">👋</span><span className="sr-only"> there</span><br />Welcome to TROVANE.<br />How can we help you today?</div></div>
