...
export function WhatsAppWidget() {
  ...
  return <aside aria-label="WhatsApp chat" className={`fixed z-[9999] ${isOpen ? "bottom-[max(16px,env(safe-area-inset-bottom))] left-4 right-4 sm:bottom-6 sm:left-auto sm:right-6" : "bottom-[max(16px,env(safe-area-inset-bottom))] right-4 sm:bottom-6 sm:right-6"}`}>
    {isOpen ? <section aria-labelledby="whatsapp-widget-title" className="ml-auto flex h-[min(450px,55dvh)] max-h-[55vh] w-full max-w-[360px] flex-col overflow-hidden rounded-[18px] border border-black/10 bg-white shadow-[0_16px_48px_rgba(20,37,31,0.24)] sm:h-[480px] sm:w-[360px]" id="whatsapp-widget">
      <header className="flex shrink-0 items-center justify-between bg-[#128C7E] px-3 py-2 text-white sm:px-4 sm:py-3">
        <div className="flex items-center gap-2.5 sm:gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-white/15 sm:h-9 sm:w-9"><WhatsAppIcon className="h-5 w-5 sm:h-6 sm:w-6" /></span><div><p className="text-[10px] font-medium text-white/75 sm:text-[11px]">TROVANE</p><h2 className="text-sm font-semibold leading-5 sm:text-base" id="whatsapp-widget-title">Chat with TROVANE</h2></div></div>
      ...
      <form className="relative shrink-0 bg-[#f0f2f5] px-2 py-2 sm:px-2.5 sm:py-2.5" onSubmit={sendMessage}>
        ...
        <div className="flex items-center gap-2"><button aria-controls="whatsapp-emoji-picker" aria-expanded={isPickerOpen} aria-label="Open emoji picker" className="flex h-11 w-11 shrink-0 touch-manipulation items-center justify-center rounded-full bg-white text-[#54656f] shadow-sm transition hover:bg-black/5 focus:outline-none focus:ring-2 focus:ring-[#25D366] sm:h-12 sm:w-12 sm:bg-transparent sm:shadow-none" onClick={() => setIsPickerOpen((open) => !open)} type="button"><SmileIcon className="h-5 w-5" /></button><label className="sr-only" htmlFor="whatsapp-message">Write your message</label><input className="h-11 min-w-0 flex-1 rounded-full bg-white px-4 text-sm text-[#182a24] outline-none placeholder:text-[#667781] focus:ring-2 focus:ring-[#25D366] sm:h-12" id="whatsapp-message" onChange={(event) => setMessage(event.target.value)} placeholder="Write your message..." ref={inputRef} type="text" value={message} /><button aria-label="Send WhatsApp message" className="flex h-11 w-11 shrink-0 touch-manipulation items-center justify-center rounded-full bg-[#25D366] text-white transition hover:bg-[#128C7E] focus:outline-none focus:ring-2 focus:ring-[#128C7E] focus:ring-offset-2 sm:h-12 sm:w-12" type="submit"><SendIcon className="h-5 w-5" /></button></div>
      </form>
