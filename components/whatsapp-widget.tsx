...
function WhatsAppIcon({ className }: { className?: string }) {
  return <svg aria-hidden="true" className={className} fill="none" viewBox="0 0 24 24"><path d="M19.1 4.9A9.8 9.8 0 0 0 3.5 16.7L2.3 21.7l5.1-1.2A9.8 9.8 0 1 0 19.1 4.9Z" fill="currentColor" /><path d="M16.8 14.7c-.2.6-1.1 1.1-1.6 1.2-.4.1-1 .2-2.9-.6-2.4-1-4-3.5-4.1-3.7-.1-.2-1-1.3-1-2.5 0-1.1.6-1.7.8-2 .2-.2.4-.2.6-.2h.4c.1 0 .3 0 .4.3l.6 1.5c.1.3.1.4 0 .6l-.3.5c-.1.2-.2.3 0 .5.2.3.8 1.3 1.8 2.1 1.2 1.1 2.2 1.4 2.5 1.6.2.1.4.1.5-.1l.6-.7c.2-.2.3-.2.6-.1l1.4.7c.2.1.4.2.4.3 0 .1 0 .6-.2 1.2Z" fill="white" /></svg>;
}

function ChevronDownIcon() {
  return <svg aria-hidden="true" fill="none" viewBox="0 0 24 24"><path d="m6 9 6 6 6-6" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" /></svg>;
}

function SmileIcon() {
  return <svg aria-hidden="true" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="8.5" stroke="currentColor" strokeWidth="1.8" /><path d="M8.5 14.1c.8 1.3 2.1 2 3.5 2s2.7-.7 3.5-2M9 9.6h.01M15 9.6h.01" stroke="currentColor" strokeLinecap="round" strokeWidth="1.8" /></svg>;
}

function SendIcon() {
  return <svg aria-hidden="true" fill="none" viewBox="0 0 24 24"><path d="m3.5 11.3 16.1-7.2-6.3 15.8-2.6-6.1-7.2-2.5Z" fill="currentColor" /><path d="m10.7 13.8 3.7-3.5" stroke="#128C7E" strokeLinecap="round" strokeWidth="1.5" /></svg>;
}
  ...
  return <aside aria-label="WhatsApp chat" className="fixed bottom-4 right-4 z-[60] sm:bottom-6 sm:right-6">
    {isOpen ? <section aria-labelledby="whatsapp-widget-title" className="flex h-[min(500px,calc(100dvh-2rem))] w-[calc(100vw-2rem)] max-w-[360px] flex-col overflow-hidden rounded-[18px] border border-black/10 bg-white shadow-[0_16px_48px_rgba(20,37,31,0.24)] sm:h-[480px]" id="whatsapp-widget">
      <header className="flex shrink-0 items-center justify-between bg-[#128C7E] px-4 py-3 text-white">
        ...
        <button aria-label="Close WhatsApp chat" className="flex h-10 w-10 items-center justify-center rounded-full text-white transition hover:bg-white/15 focus:outline-none focus:ring-2 focus:ring-white" onClick={() => { setIsOpen(false); setIsPickerOpen(false); }} type="button"><ChevronDownIcon /></button>
      </header>
      ...
      <form className="relative shrink-0 bg-[#f0f2f5] px-2.5 py-2.5" onSubmit={sendMessage}>
        ...
        <div className="flex items-center gap-2"><button aria-controls="whatsapp-emoji-picker" aria-expanded={isPickerOpen} aria-label="Open emoji picker" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-[#54656f] transition hover:bg-black/5 focus:outline-none focus:ring-2 focus:ring-[#25D366]" onClick={() => setIsPickerOpen((open) => !open)} type="button"><SmileIcon /></button><label className="sr-only" htmlFor="whatsapp-message">Write your message</label><input className="h-11 min-w-0 flex-1 rounded-full bg-white px-4 text-sm text-[#182a24] outline-none placeholder:text-[#667781] focus:ring-2 focus:ring-[#25D366]" id="whatsapp-message" onChange={(event) => setMessage(event.target.value)} placeholder="Write your message..." ref={inputRef} type="text" value={message} /><button aria-label="Send WhatsApp message" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#25D366] text-white transition hover:bg-[#128C7E] focus:outline-none focus:ring-2 focus:ring-[#128C7E] focus:ring-offset-2" type="submit"><SendIcon /></button></div>
      </form>
    </section> : <button aria-controls="whatsapp-widget" aria-expanded="false" aria-label="Open WhatsApp chat" className="flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_10px_24px_rgba(37,211,102,0.34)] transition duration-200 hover:-translate-y-0.5 hover:bg-[#128C7E] focus:outline-none focus:ring-2 focus:ring-[#128C7E] focus:ring-offset-2" onClick={() => setIsOpen(true)} type="button"><WhatsAppIcon className="h-8 w-8" /></button>}
  </aside>;
}
